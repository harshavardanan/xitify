...
const xitLineModifierPatterns = {
  ...
  priority: /[!.]{1,} /gm,
  dueDate:
    /-> ([0-9]{4}(-|\/){0,1}([qQwW]{0,1}[0-9]{1,2}){0,1})(-|\/){0,1}([0-9]{2}){0,1}/gm,
  tag: /#[^ ]{1,}/gm,
};
...
function toObject(xitString) {
  ...
  const parseXitModifiers = (content) => {
    ...
    const priorityLevel = hasPriority
      ? content
          .match(xitLineModifierPatterns.priority)[0]
          .trim()
          .replace(/\.*/gm, "").length
      : 0;
    const priorityPadding = hasPriority
      ? content
          .match(xitLineModifierPatterns.priority)[0]
          .trim()
          .replace(/\!*/gm, "").length
    ...
    if (due !== null && due.length) modifiers.due = due[0].split("-> ")[1];

    if (tags !== null && tags.length) modifiers.tags = tags;
  ...
  const addXitObjectGroupLine = (uuid, type, status, content) => {
      ...
      switch (status) {
        case ITEM_STATUS_OPEN:
          readableContent = readableContent.replace(
            xitItemStatusDelimiterPatterns.openItem,
            ""
          );
          break;
        case ITEM_STATUS_CHECKED:
          readableContent = readableContent.replace(
            xitItemStatusDelimiterPatterns.checkedItem,
            ""
          );
          break;
        case ITEM_STATUS_ONGOING:
          readableContent = readableContent.replace(
            xitItemStatusDelimiterPatterns.ongoingItem,
            ""
          );
          break;
        case ITEM_STATUS_OBSOLETE:
          readableContent = readableContent.replace(
            xitItemStatusDelimiterPatterns.obsoleteItem,
            ""
          );
          break;
        case ITEM_STATUS_IN_QUESTION:
          readableContent = readableContent.replace(
            xitItemStatusDelimiterPatterns.inQuestionItem,
            ""
          );
          break;
      }
    ...
    if (type === ITEM_TYPE || type === ITEM_DETAILS_TYPE) {
      readableContent = readableContent.replace(
        xitLineModifierPatterns.priority,
        ""
      );
      readableContent = readableContent.replace(
        xitLineModifierPatterns.dueDate,
        ""
      );
      readableContent = readableContent.replace(
        xitLineModifierPatterns.tag,
        ""
      );
    }
    ...
    xitObject.groups[uuid].push({
      ...
      modifiers:
        type === TITLE_TYPE || type === NEWLINE_TYPE
          ? null
          : parseXitModifiers(trimmedRawContent),
      groupID: uuid,
    });
  };
  ...
  xitString.split("\n").forEach((line, idx) => {
    if (line.match(xitLineTypePatterns.title)) {
      ...
      (prevItemType === ITEM_TYPE || prevItemType === ITEM_DETAILS_TYPE) &&
      line.match(xitLineTypePatterns.itemDetails)
    ) {
    ...
    Object.values(group).forEach((line) => {
      if (line.type === TITLE_TYPE) {
        ...
        if (line.modifiers.hasPriority) {
          for (
            let priorityPadding = 1;
            priorityPadding <= line.modifiers.priorityPadding;
            priorityPadding++
          ) {
            xitString += `.`;
          }
          for (
            let priorityLevel = 1;
            priorityLevel <= line.modifiers.priorityLevel;
            priorityLevel++
          ) {
            xitString += `!`;
          }
